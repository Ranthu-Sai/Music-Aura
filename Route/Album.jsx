...
export const Album = ({route}) => {
  ...
  const {id, highlightSongId} = route.params;

  async function fetchAlbumData() {
    try {
      setLoading(true);
      const {isSong} = route.params;
      let data = {data: {songs: []}};
          ...
          if (song && (song.name || song.title)) {
            return {
              data: {
                ...
                year: song.year,
                songs: [song],
                primaryArtist: FormatArtist(
                  song.artists?.primary || song.artist,
                ),
              },
            };
          }
      ...
      if (
        albumType.includes('podcast') ||
        albumType.includes('show') ||
        albumName.includes('podcast') ||
        albumName.includes('episode')
      ) {
        data = {
          data: {
    ...
    <MainWrapper>
      ...
      {!Loading && (
        <>
          {(() => {
            const playableSongs =
              Data?.data?.songs?.filter(
                song =>
                  !song.name?.toLowerCase()?.includes('trailer') &&
                  !song.name?.toLowerCase()?.includes('sample'),
              ) || [];
