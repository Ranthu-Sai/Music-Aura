...
export async function getAppStorageDynamics() {
  try {
    ...
    // Get all cache files
    const allCacheFiles = await ReactNativeBlobUtil.fs
      .ls(dirs.CacheDir)
      .catch(() => []);
    ...
    for (const fileName of allCacheFiles) {
      ...
      if (imageCachePaths.some(p => p.includes(fileName))) {
        continue;
      }

      try {
        const stats = await ReactNativeBlobUtil.fs.stat(filePath);
        const size = parseInt(stats.size, 10) || 0;
        ...
        if (stats.type === 'directory') {
          ...
          if (['jpg', 'jpeg', 'png', 'webp', 'gif'].includes(ext)) {
            ...
            // < 100KB likely metadata or images
            imageCacheSize += size;
          } else {
            // Larger files likely audio chunks
            songCacheSize += size;
          }
        }
      } catch (statErr) {
