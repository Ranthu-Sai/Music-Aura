...
class AutoRecommendations {
    ...
    async fetchAndAppendRecommendations() {
        ...
        try {
            ...
            // CRITICAL: Validate URLs before adding to TrackPlayer
            const { validateTracks } = require('../Utils/URLValidator');
            const validatedSongs = validateTracks(formattedSongs);

            if (validatedSongs.length === 0) {
                console.warn('⚠️ AutoRecommendations: No valid tracks after validation');
                this.isFetching = false;
                return;
            }
