...
class NavigationCacheManager {
  ...
  getStreamUrl(videoId, source = 'ytmusic') {
    ...
    if (!entry.url || typeof entry.url !== 'string') {
      console.warn(
        `[CacheManager] Invalid cached URL type for ${source}:${videoId}, clearing cache`,
      );
      this.streamCache.delete(key);
      return null;
    }
    ...
    if (!url) {
      console.warn(
        `[CacheManager] Empty cached URL for ${source}:${videoId}, clearing cache`,
      );
      this.streamCache.delete(key);
      return null;
    }
    ...
    if (!url.startsWith('http://') && !url.startsWith('https://')) {
      console.warn(
        `[CacheManager] Invalid cached URL protocol for ${source}:${videoId}: ${url.substring(
          0,
          50,
        )}, clearing cache`,
      );
  ...
  setStreamUrl(videoId, url, source = 'ytmusic') {
    ...
    if (typeof url !== 'string') {
      console.warn(
        `[CacheManager] Cannot cache stream URL: url must be a string, got ${typeof url}`,
      );
      return;
    }
    ...
    if (
      ...
      console.warn(
        `[CacheManager] Cannot cache stream URL: invalid protocol: ${trimmedUrl.substring(
          0,
          50,
        )}`,
      );
    ...
    debugLog(
      `[CacheManager] Stream URL cached for ${source}:${videoId} (TTL: ${
        ttl / 1000 / 60
      } minutes)`,
    );
  }
