...
import { ToastAndroid, Platform, PermissionsAndroid, DeviceEventEmitter } from "react-native";
    ...
    try {
        ...
        if (typeof urlToResolve === 'string' && urlToResolve) {
            if (!urlToResolve.startsWith('http')) {
                const streamData = await youtubeStreamingService.getStreamUrl(urlToResolve);
...
async function finalizeDownload(song, res, fileName, mime, source, showSystemNotification = true) {
    const filePath = res.path();
    console.log('DownloadHelper: Finalizing download for', fileName, 'at', filePath);
    ...
    try {
        ...
        if (Platform.OS === 'android' && showSystemNotification) {
            try {
                ...
                await ReactNativeBlobUtil.android.addCompleteDownload(notificationConfig);
                console.log('DownloadHelper: System notification triggered successfully');
            } catch (notifyErr) {
                console.warn('DownloadHelper: System notification failed, trying fallback path format:', notifyErr);
                try {
        ...
        const songId = song.id || `${song.title}-${song.artist}`.replace(/[^a-zA-Z0-9]/g, '_');
        const metadata = {
            id: songId,
            title: song.title,
            artist: song.artist,
...
export async function DownloadSong(song) {
    try {
        ...
        console.log('DownloadHelper: Resolving path for', song.title, 'with base preference:', path);

        const deriveExt = () => {
            const lower = (downloadLink || '').toLowerCase();
            // Force .mp3 for YouTube/GoogleVideo to satisfy user preference for MP3 format
            if (downloadLink.includes('googlevideo.com') || downloadLink.includes('youtube')) {
                return 'mp3';
            }
        ...
        if (!baseDir || baseDir.includes('data/user') || baseDir.includes('com.music_aura')) {
            baseDir = '/storage/emulated/0/' + (path === "Downloads" ? "Download" : "Music");
        }
        ...
        try {
            ...
            console.warn('DownloadHelper: Failed to create directory:', musicAuraDir, e);
        }
        ...
        if (isYouTubeUrl) {
            ...
            const startDownload = (useManager = true) => {
                ...
                return ReactNativeBlobUtil.config(config).fetch('GET', downloadLink, {});
            };
                    ...
                    if (err.message && (err.message.includes('null') || err.message.includes('FileStorage'))) {
                        ...
                        startDownload(false)
                            .then(async (res) => {
                                ...
                                await finalizeDownload(song, res, fileName, mime, 'other', true);
                            })
                            .catch((fallbackErr) => {
                                console.error('DownloadHelper: Fallback download failed:', fallbackErr);
                                ToastAndroid.show(`Download failed: ${fallbackErr.message || 'Unknown error'}`, ToastAndroid.LONG);
                            });
                    } else {
