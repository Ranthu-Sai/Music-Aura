...
class QueueManager {
  ...
  async buildQueueFromRecommendations(songId, source = 'saavn', limit = 10) {
    try {
      ...
      if (
        source === 'ytmusic' ||
        (typeof songId === 'string' && songId.length === 11)
      ) {
        ...
        const queueSongs = nextData.items
          .filter(song => {
            ...
            return (
              videoId && typeof videoId === 'string' && videoId.length === 11
            );
          })
          .slice(0, limit)
          .map(song => {
  ...
  async _onTrackChange(event) {
    ...
    try {
      ...
      debugLog(
        `📊 Queue status: ${songsRemaining} songs remaining after current`,
      );
        ...
        try {
          ...
          if (recommendations && recommendations.length > 0) {
            ...
            if (newSongs.length > 0) {
              ...
              debugLog(
                `✅ Added ${newSongs.length} more songs to extend queue!`,
              );
            } else {
