...
async function getPlaylistData(id){
  ...
  if (id.startsWith('/')) {
    ...
    // STRATEGY 2: Fallback to Piped API
    ...
    for (let baseUrl of urls) {
      try {
        ...
        const songs = response.data.relatedStreams.filter(video => video.url).map(video => ({
          ...
          downloadUrl: video.url ? video.url.replace('/watch?v=', '') : '',
    ...
    throw new Error('Failed to fetch YouTube playlist from all sources');
  } else {
    // Saavn playlist
      ...
      for (const section of contents) {
        ...
        for (const item of musicShelf.contents) {
          ...
          if (songThumbnail && (songThumbnail.includes('googleusercontent.com') || songThumbnail.includes('ggpht.com'))) {
            songThumbnail = songThumbnail.split('=')[0] + '=w1080-h1080-l90-rj';
          }
