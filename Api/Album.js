...
async function getYTMusicAlbumData(browseId) {
  try {
    ...
    const tabContent =
      twoCol?.tabs?.[0]?.tabRenderer?.content?.sectionListRenderer;
    ...
    const songs = [];
    const musicShelf =
      twoCol?.secondaryContents?.sectionListRenderer?.contents?.[0]
        ?.musicShelfRenderer;
    ...
    if (musicShelf && musicShelf.contents) {
      ...
      for (const item of musicShelf.contents) {
        ...
        // Extract title
        const title =
          musicItem.flexColumns?.[0]?.musicResponsiveListItemFlexColumnRenderer
            ?.text?.runs?.[0]?.text || 'Unknown';

        // Extract artist
        const artistRuns =
          musicItem.flexColumns?.[1]?.musicResponsiveListItemFlexColumnRenderer
            ?.text?.runs;
        const artist =
          artistRuns
            ?.filter(r => r.text !== ' • ')
