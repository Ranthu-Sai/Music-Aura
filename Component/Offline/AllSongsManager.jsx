...
const useAllSongsManager = ({
    ...
    try {
      ...
      for (const [songId, metadata] of Object.entries(allMetadata)) {
        try {
          ...
          console.warn(
            `useAllSongsManager: Error processing downloaded song ${songId}:`,
            error,
          );
        }
      }
      ...
      console.error(
        'useAllSongsManager: Error loading downloaded songs:',
        error,
      );
      return [];
    } finally {
  ...
  const removeDownloadedSong = useCallback(
    async songId => {
      try {
        ...
        console.error(
          'useAllSongsManager: Error removing downloaded song:',
          error,
        );
        return false;
      }
  ...
  // Remove a local song
  const removeLocalSong = useCallback(
    async songId => {
      removeTrack(songId);
      return true;
    },
    ...
    try {
      ...
      if (Platform.OS === 'android') {
        ...
        if (permissionGranted) {
          ToastAndroid.show(
            'Permission granted! Loading songs...',
            ToastAndroid.SHORT,
          );
          ...
          if (shouldShowSettings) {
            ...
            ToastAndroid.show(
              'Permission denied. Please grant storage permission to access songs.',
              ToastAndroid.LONG,
            );
          }
        }
  ...
  const getDownloadStats = useCallback(() => {
    return {
      ...
      totalSize: downloadedSongs.reduce(
        (sum, song) => sum + (song.fileSize || 0),
        0,
      ),
