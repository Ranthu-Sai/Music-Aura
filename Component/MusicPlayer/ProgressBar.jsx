...
export const ProgressBar = memo(() => {
  ...
  const accurateDuration = Number.isFinite(currentTrack?.duration) && currentTrack?.duration > 0
    ? currentTrack.duration
    : (stableDurationRef.current > 0 ? stableDurationRef.current : (Number.isFinite(duration) ? duration : 0));
  ...
  return (
    <>
      <Slider
        ...
        onSlidingStart={() => {
          const playing = playbackState?.state === 3 || playbackState === 3 || playbackState?.state === 'playing';
          setWasPlaying(Boolean(playing));
          setIsSliding(true);
        }}
        onSlidingComplete={async (value) => {
          try {
            const target = Math.max(0, Math.min(value ?? 0, accurateDuration || 0));
