  ...
  const showInfo = () => {
    Alert.alert(
      ...
      `Title: ${Visible.title}\nArtist: ${Visible.artist}\nAlbum: ${
        Visible.albumName || 'N/A'
      }\nDuration: ${Math.floor(Visible.duration / 60)}:${(
        Visible.duration % 60
      )
        .toString()
    ...
    <Modal
      ...
      <View
        ...
        <View
          ...
          <EachModalButton
            text={'Info'}
            icon={<AntDesign name={'infocirlceo'} size={25} color={'white'} />}
            Onpress={showInfo}
          />
        </View>
        <Spacer />
        <View
            ...
            <EachModalButton
              ...
              Onpress={async () => {
                ...
                try {
                  ...
                  if (songPath) {
                    const exists =
                      (await ReactNativeBlobUtil.fs.exists(songPath)) ||
                  ...
                  if (deletedSuccessfully) {
                    ToastAndroid.show(
                      'Song deleted from storage',
                      ToastAndroid.SHORT,
                    );
                  } else {
                    ToastAndroid.show(
                      'Removed from list, but file may still exist in storage',
                      ToastAndroid.LONG,
                    );
                  }
            ...
            <EachModalButton
              ...
              Onpress={async () => {
                const historyManager =
                  require('../../Utils/HistoryManager').default;
                const success = await historyManager.removeFromHistory(
                  Visible.id,
                );
                if (success) {
          ...
          {Visible.playlistId && (
            <EachModalButton
              ...
              Onpress={async () => {
                const {
                  RemoveFromPlaylist,
                } = require('../../LocalStorage/StoreUserPlaylists');
          ...
          {Visible.source === 'local' ||
          (Visible.url &&
            typeof Visible.url === 'string' &&
            (Visible.url.startsWith('/') ||
              Visible.url.startsWith('file://')) &&
            Visible.source !== 'downloaded') ? (
            <EachModalButton
              text={'Delete'}
              colors={['#e53935', '#e35d5b']}
          ...
          {Visible.albumId && (
            <EachModalButton
              ...
              Onpress={() => {
                setVisible({visible: false});
                Visible.navigation.navigate('Album', {
                  id: Visible.albumId,
                  image: Visible.image,
                });
              }}
