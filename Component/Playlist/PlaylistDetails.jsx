...
export const PlaylistDetails = ({ name, listener, notReleased, Data, Loading, id, image, follower }) => {
  ...
  async function AddToPlayer() {
    ...
    const ForMusicPlayer = Data?.data?.songs?.map((e, i) => {
      ...
      return {
        url: isLocal ? e.url : (Array.isArray(e?.downloadUrl) ? e?.downloadUrl[quality]?.url : e?.downloadUrl),
        title: FormatTitleAndArtist(e?.name || e?.title),
        artist: FormatTitleAndArtist(FormatArtist(e?.artists?.primary) || e?.artist || 'Unknown Artist'),
        artwork: isLocal ? e.image : (Array.isArray(e?.image) ? e?.image[2]?.url : e?.image),
        image: isLocal ? e.image : (Array.isArray(e?.image) ? e?.image[2]?.url : e?.image),
        duration: e?.duration,
        id: e?.id,
        language: e?.language,
        ...
        <View style={{
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 15 }}>
            ...
            {id?.startsWith('playlist_') && (
              <TouchableOpacity onPress={handleDeletePlaylist} style={{ padding: 5 }}>
                <MaterialCommunityIcons name="delete-outline" size={24} color="#FF4B2B" />
              </TouchableOpacity>
            )}
          </View>
        </View>
